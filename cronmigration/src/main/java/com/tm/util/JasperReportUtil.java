...
public class JasperReportUtil {
	...
	public byte[] createReportFromJasperTemplate(JRBeanCollectionDataSource datasource,
			String templatePath, String reportType, Map<String, Object> parameters) {
		JasperPrint jasperPrint;
		byte[] bytes = null;
		try {
			if (reportType.equalsIgnoreCase(PDF)) {
				bytes = JasperRunManager.runReportToPdf(jasperReport.concat(templatePath), parameters, datasource);
			}
			if (reportType.equalsIgnoreCase(XLS) || reportType.equalsIgnoreCase(XLSX)) {
				ByteArrayOutputStream output = new ByteArrayOutputStream();
				jasperPrint = JasperFillManager.fillReport(jasperReport.concat(templatePath), parameters, datasource);
	...
	public byte[] createReportFromJasperTemplateEmptyDatasoruce(JREmptyDataSource datasource,
			String sourceFileName, String reportType, Map<String, Object> parameters) {
		JasperPrint jasperPrint;
		byte[] bytes = null;
		try {
			
			if (reportType.equalsIgnoreCase(JasperReportUtil.PDF)) {
				bytes = JasperRunManager.runReportToPdf(jasperReport.concat(sourceFileName), parameters, datasource);
			}
			if (reportType.equalsIgnoreCase(JasperReportUtil.XLS)
					|| reportType.equalsIgnoreCase(JasperReportUtil.XLSX)) {

				ByteArrayOutputStream output = new ByteArrayOutputStream();
				jasperPrint = JasperFillManager.fillReport(sourceFileName, parameters, datasource);
