...
/**
 * A {@link TokenStore} implementation that just reads data from the tokens themselves. Not really a
 * store since it never persists anything, and methods like
 * {@link #getAccessToken(OAuth2Authentication)} always return null. But nevertheless a useful tool
 * since it translates access tokens to and from authentications. Use this wherever a
 * {@link TokenStore} is needed, but remember to use the same {@link JwtAccessTokenConverter}
...
public class OIDCTokenStore implements TokenStore {
    ...
    private OIDCAccessTokenConverter oidcTokenEnhancer;

    private ApprovalStore approvalStore;

    /**
     * Create a OIDCTokenStore with this token enhancer (should be shared with the
     * DefaultTokenServices if used).
     * 
     * @param oidcTokenEnhancer
     */
    public OIDCTokenStore(OIDCAccessTokenConverter jwtTokenEnhancer) {
        this.oidcTokenEnhancer = jwtTokenEnhancer;
    }
    ...
    public OAuth2Authentication readAuthentication(String token) {
        final JWTClaimsSet claims = readClaims(token);
        return oidcTokenEnhancer.extractAuthentication(claims.getClaims());
    }

    private JWTClaimsSet readClaims(String token) {
        RequestContextHolder.getRequestAttributes().setAttribute("token", token,
                RequestAttributes.SCOPE_REQUEST);
        return oidcTokenEnhancer.readClaims(token);
    }
    ...
    public OAuth2AccessToken readAccessToken(String tokenValue) {
        ...
        if (oidcTokenEnhancer.isRefreshToken(accessToken)) {
    ...
    private OAuth2AccessToken convertAccessToken(String tokenValue) {
        final JWTClaimsSet claims = readClaims(tokenValue);
        return oidcTokenEnhancer.extractAccessToken(tokenValue, claims.getClaims());
    }
    ...
    private OAuth2RefreshToken createRefreshToken(OAuth2AccessToken encodedRefreshToken) {
        if (!oidcTokenEnhancer.isRefreshToken(encodedRefreshToken)) {
    ...
    public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
        // We don't want to accidentally issue a token, and we have no way to reconstruct the
        // refresh token
        return null;
    }
    ...
    public void setTokenEnhancer(OIDCAccessTokenConverter tokenEnhancer) {
        this.oidcTokenEnhancer = tokenEnhancer;
    }
