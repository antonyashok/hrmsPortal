...
@ControllerAdvice
public class ManualInvoiceExceptionTranslator {

    public static final String FILE_SIZE_EXCEED = "error.fileSize.value";

    public ManualInvoiceExceptionTranslator(MessageSource messageSource) {
        new MessageSourceAccessor(messageSource);
    }

    @SuppressWarnings("rawtypes")
    @ExceptionHandler(ManualInvoiceException.class)
    public ResponseEntity handleFileUploadException(ManualInvoiceException response)
            throws IOException {
